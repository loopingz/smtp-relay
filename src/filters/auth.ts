...
import { SmtpFlow } from "../flow"; // Import SmtpFlow
...
export class AuthFilter extends SmtpFilter<AuthFilterConfig> {
  ...
  constructor(flow: SmtpFlow, config: AuthFilterConfig, logger: WorkerOutput) {
    super(flow, config, logger); // Corrected call to super
    this.mtaHostname = config.mtaHostname || os.hostname();
    this.mailauthOptions = config.mailauthOptions || {};
    this.logger.log("INFO", "AuthFilter initialized with config: %j, mtaHostname: %s, mailauthOptions: %j", this.config, this.mtaHostname, this.mailauthOptions);
  ...
  async onDataStream(stream: SMTPServerDataStream, session: SmtpSession, callback: (err?: Error | null, code?: string) => void): Promise<void> {
    this.logger.log("INFO", "AuthFilter onDataStream called for session: %s with mailauthOptions: %j", session.id, this.mailauthOptions);
    const chunks: Buffer[] = [];
    ...
    stream.on("end", async () => {
      ...
      try {
        ...
        const mailFrom = session.envelope.mailFrom && session.envelope.mailFrom.address ? session.envelope.mailFrom.address : ""; // Ensure mailFrom.address is accessed safely

        if (!remoteIp) {
          this.logger.log("WARN", "Remote IP address is not available, skipping authentication. Session: %s", session.id);
          return callback(); // Accept if IP is missing (or handle as per policy)
        }

        this.logger.log("INFO", `Performing email authentication for IP: ${remoteIp}, HELO: ${heloHostname}, From: ${mailFrom}, MTA: ${this.mtaHostname}`);
        ...
        this.logger.log("INFO", "MailAuth verification results for session %s: DKIM: %s, SPF: %s, DMARC: %s",
          session.id,
        ...
        if (spfResult && spfResult !== "pass" && spfResult !== "none" && spfResult !== "neutral") { // none and neutral are often treated as acceptable
          if (this.config.spf?.policy === "reject") {
            this.logger.log("WARN", `SPF check failed with result '${spfResult}' and policy is 'reject'. Rejecting email from ${mailFrom} (session ${session.id}).`);
            ...
            this.logger.log("INFO", `SPF check failed with result '${spfResult}' and policy is 'tag'. Email from ${mailFrom} will be tagged (session ${session.id}).`);
        ...
        if (dkimFirstResult && dkimFirstResult !== "pass") {
          if (this.config.dkim?.policy === "reject") {
            this.logger.log("WARN", `DKIM check failed with result '${dkimFirstResult}' and policy is 'reject'. Rejecting email from ${mailFrom} (session ${session.id}).`);
            ...
            this.logger.log("INFO", `DKIM check failed with result '${dkimFirstResult}' and policy is 'tag'. Email from ${mailFrom} will be tagged (session ${session.id}).`);
        ...
        this.logger.log("INFO", "Email authentication passed or policies allow. Accepting email from %s (session %s)", mailFrom, session.id);
        ...
        this.logger.log("ERROR", "Error during email authentication for session %s: %s", session.id, error.message, error.stack);
    ...
    stream.on("error", (err: Error) => {
      this.logger.log("ERROR", "Error in email data stream for session %s: %s", session.id, err.message, err.stack);
