...
class AuthFilterSmtpServerTest {
  ...
  private async setupServer(configPath: string): Promise<void> {
    console.log(`[AuthFilterSmtpServerTest SETUP] Using configPath: ${configPath}`);
    this.server = new SmtpServer(configPath);
    console.log(`[AuthFilterSmtpServerTest SETUP] SmtpServer instance created. Initializing...`);
    await this.server.init(); // Now correctly awaits the server listening
    console.log(`[AuthFilterSmtpServerTest SETUP] SmtpServer initialized and listening.`);
    // No need for arbitrary setTimeout if init() resolves on listening
  }
  ...
  async "DKIM Fail, Policy Reject: Email should be rejected"(done) {
    ...
    try {
      // Pass the expected error code to failEmail
      await this.smtpTest.failEmail("DATA", FROM_ADDRESS, TO_ADDRESS, EMAIL_BODY, "550"); 
      // We only check the code 550, the SmtpTest helper will check if the message contains the expected text from AuthFilter
      // For a more precise check, SmtpTest.failEmail would need to allow asserting the full error message.
      // For now, checking the code is a good step. The actual error message is "DKIM check failed: DKIM verification failed"
      // and the SmtpTest helper might not be set up to assert the exact string.
      // Let's assume the SmtpTest helper's default error message check is sufficient if the code matches.
      // If tests still fail on message mismatch, I'll need to adjust SmtpTest or the assertion here.
      // The failEmail method in SmtpTest does not currently assert the *text* of the error, only the code.
      // So, this should be okay. My AuthFilter sends: callback(new Error(MESSAGE), CODE)
      done();
    } catch (e) {
      done(e);
    }
  ...
  async "DKIM & SPF Fail, Policy Accept: Email should be accepted"(done) {
    ...
    try {
      // Policy is "accept", so email should be sent successfully despite auth failures
      await this.smtpTest.sendEmail(FROM_ADDRESS, TO_ADDRESS, EMAIL_BODY);
      done();
    } catch (e) {
      done(e);
  ...
  async "DKIM & SPF Pass, Policy Reject: Email should be accepted"(done) {
    ...
    try {
      // This test is for "accept" policy, so it should sendEmail, not failEmail
      await this.smtpTest.sendEmail(FROM_ADDRESS, TO_ADDRESS, EMAIL_BODY); 
      done();
    } catch (e) {
      done(e);
